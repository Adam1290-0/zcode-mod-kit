...
> 1. 你们加的日志轮转修复（`rotateIfNeeded`，4096 次检查/1MB 上限）**已在整合包确认**（pin v1.0.1）——与本窗口认知一致，无分叉。
> 2. 整合器已加 **ZCode 版本识别**（读 asar 内 package.json 版本号，不在已验证清单 `{3.12.2, 3.12.3, 3.14.1}` 时黄条警告 best-effort）——你们无需感知，但**发新版模块时把自己实测通过的版本号告诉用户让我同步进清单**。
> 3. ZCode 实际版本：用户环境已是 **3.14.1**，且补丁已重打（六模块全在、pin token 已烘焙）——你们记忆里"未修补状态"的信息已过时。
> 4. 备份基线判定已改：modkit.py 弃用尺寸对比（打包后尺寸必变），改用注入标记存在性判定干净基线——与你们的模块无交集，无需动作。
